...
static int Lua_Music_Tempo_Set(lua_State* L)
{
	if (!lua_isnumber(L, 2))
		return luaL_error(L, "tempo: incorrect argument type");

	auto tempo = static_cast<int>(lua_tonumber(L, 2));
	if (tempo < -12 || tempo > 12) return luaL_error(L, "tempo: invalid operation");

	int index = Lua_Music::Index(L, 1) + Music::reserved_music_slots;
	auto slot = Music::instance()->GetSlot(index);
	if (!slot) return luaL_error(L, "tempo: index out of bounds");

	slot->SetTempo(tempo);
	return 0;
}

static int Lua_Music_Tempo_Get(lua_State* L)
{
	int index = Lua_Music::Index(L, 1) + Music::reserved_music_slots;
	auto slot = Music::instance()->GetSlot(index);
	if (!slot) return luaL_error(L, "tempo: index out of bounds");

	lua_pushnumber(L, static_cast<int>(slot->GetParameters().tempo));
	return 1;
}
...
char Lua_Music_Preset_Name[] = "music_preset";
char Lua_Music_Segment_Name[] = "music_segment";
