...
static int Lua_MusicSegment_Map_Segment_Transition(lua_State* L)
{
	...
	MusicPlayer::Segment::Mapping mapping(next_segment_index, { fadeTypeOut , fadeTypeOutSeconds }, { fadeTypeIn , fadeTypeInSeconds }, crossfade);

	if (!slot->SetSegmentMapping(preset_index, segment_index, next_preset_index, mapping))
...
static int Lua_Music_CurrentSegment(lua_State* L)
{
	int index = Lua_Music::Index(L, 1) + Music::reserved_music_slots;
	auto slot = Music::instance()->GetSlot(index);
	if (!slot) return luaL_error(L, "current_segment: index out of bounds");

	auto [preset_index, segment_index] = slot->GetCurrentSegment();
	auto x = Get_LuaMusicSegment(index, preset_index, segment_index).value();
	Lua_MusicSegment::Push(L, x);
	return 1;
}
...
const luaL_Reg Lua_Music_Get[] = {
	...
	{"current_segment", Lua_Music_CurrentSegment },
	{0, 0}
};
