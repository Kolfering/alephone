...
bool MusicPlayer::ProcessTransitionIn(uint8* data, uint32_t length, uint32_t next_preset_index, std::optional<std::pair<Segment::Mapping, SegmentTransitionOffsets>> segment_mapping_offsets) {

	if (!segment_mapping_offsets.has_value()) return false;

	const auto& transition = segment_mapping_offsets.value();
	const auto mapping = transition.first;
	const auto transitionOffsets = transition.second;
	...
	if (mapping.transition_in.fade_type != FadeType::None && transitionOffsets.second.has_value()) {
		...
		if (currentPosition < fadeInTransitionOffset) {
			const auto fadedDataLength = std::min(fadeInTransitionOffset - currentPosition, length);
			const auto startFadeOffset = currentPosition; // fade-in always starts at 0
...
bool MusicPlayer::ProcessTransition(uint8* data, uint32_t length, uint32_t next_preset_index, std::optional<Segment::Mapping> segment_mapping) {
	...
	if (segment_mapping.has_value()) { 	//out
		...
		if (mapping.transition_out.fade_type != FadeType::None && transitionOffsets.first.has_value()) {
			...
			if (currentPosition > fadeOutTransitionOffset) {
				...
				const auto startFadeOffset = currentPosition - fadeOutTransitionOffset;
				const auto fadedDataLength = std::min(length - startFadeOffset, length);
				const auto bufferOffset = startFadeOffset < length ? startFadeOffset : 0;

				ApplyFade(mapping.transition_out.fade_type, false, fadeTotalLength, startFadeOffset, data + bufferOffset, fadedDataLength);
				...
				if (mapping.crossfade) {
					...
					const bool transitionOutDone = transitionDone;
					...
					transitionDone &= ProcessTransitionIn(crossfade_fadeInData.data(), length, next_preset_index, std::make_pair(mapping, transitionOffsets));
					CrossFadeMix(data, crossfade_fadeInData.data(), length);
					if (transitionOutDone) SwitchSegment(segment_mapping);
				}
	...
	if (previous_transition_preset_index.has_value() && previous_transition_segment_index.has_value()) { //in

		const auto previousPresetIndex = previous_transition_preset_index.value();
		const auto previousSegmentIndex = previous_transition_segment_index.value();
		const auto previousMapping = music_presets[previousPresetIndex].GetSegment(previousSegmentIndex)->GetNextSegmentMapping(current_preset_index);
		if (!previousMapping.has_value()) return false;

		const auto& previousMappingValue = previousMapping.value();
		const auto mappingTransitions = std::make_pair(previousMappingValue, ComputeTransitionOffsets(current_preset_index, previousMappingValue));
		return ProcessTransitionIn(data, length, current_preset_index, mappingTransitions);
	}
...
void MusicPlayer::ApplyFade(FadeType fade_type, bool fade_in, uint32_t fade_total_length, uint32_t current_position, uint8* data, uint32_t faded_data_length) {
	...
	for (uint32_t i = 0; i < samplesToFade; ++i) {
		...
		float t = static_cast<float>(offsetSamples + i) / static_cast<float>(fadeTotalSamples);
		float factor;
...
void MusicPlayer::CrossFadeMix(uint8* data_out, uint8* data_in, uint32_t length) {
	...
	const auto totalSamples = length / sizeof(float);
...
uint32_t MusicPlayer::GetNextData(uint8* data, uint32_t length) {
	...
	SwitchSegment(nextSegmentMapping);
	return HasBufferFormatChanged() ? dataSize : dataSize + GetNextData(data + dataSize, length - dataSize);
}

void MusicPlayer::SwitchSegment(std::optional<Segment::Mapping> segment_mapping) {

	current_decoder->Rewind();
	previous_transition_preset_index = std::nullopt;
	previous_transition_segment_index = std::nullopt;

	if (!segment_mapping.has_value()) return;

	const auto& nextSegmentMappingValue = segment_mapping.value();
	previous_transition_preset_index = current_preset_index;
	previous_transition_segment_index = current_segment_index;
	current_segment_index = nextSegmentMappingValue.segment_id;
