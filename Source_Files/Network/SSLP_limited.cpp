...
SSLPint_FoundAnInstance(struct SSLP_ServiceInstance* inInstance) {
    ...
    while(theCurrentFoundInstance != NULL) {
        if(inInstance->sslps_address.address() == theCurrentFoundInstance->mInstance->sslps_address.address()) {
...
SSLPint_LostAnInstance(struct SSLP_ServiceInstance* inInstance) {
    ...
    while(theCurrentInstance != NULL) {
        if(theCurrentInstance->mInstance->sslps_address.address() == inInstance->sslps_address.address()) {
...
SSLPint_Enter() {
    ...
    sSocketDescriptor = NetGetNetworkManager()->open_udp_socket(SSLP_PORT);
