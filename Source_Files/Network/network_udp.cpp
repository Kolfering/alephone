...
static std::unique_ptr<UDPsocket> sSocket = nullptr;
...
receive_thread_function(void*) {
	...
	while (sKeepListening) {
		...
		if (sSocket->check() > 0 && take_mytm_mutex()) {

			if (sSocket->receive(packet) > 0) {
...
bool NetDDPOpenSocket(uint16_t ioPortNumber, PacketHandlerProcPtr packetHandler)
{
	...
	sSocket = NetGetNetworkManager()->open_udp_socket(ioPortNumber);
