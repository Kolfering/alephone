...
public:
	...
	typedef std::pair<std::optional<uint32_t>, std::optional<uint32_t>> SegmentTransitionOffsets;
	...
	public:
		...
		struct Mapping {
			uint32_t segment_id;
			TransitionData transition_out;
			TransitionData transition_in;
			bool crossfade;
		
			Mapping(uint32_t segment_id, TransitionData transition_out, TransitionData transition_in, bool crossfade) :
...
private:
	...
	bool ProcessTransitionIn(uint8* data, uint32_t length, uint32_t next_preset_index, std::optional<std::pair<Segment::Mapping, SegmentTransitionOffsets>> segment_mapping);
	...
	void SwitchSegment(std::optional<Segment::Mapping> segment_mapping);
	...
	std::optional<uint32_t> previous_transition_segment_index;
	std::optional<uint32_t> previous_transition_preset_index;
